...
function analiza_komentari(response,vrzta,diid) {
  ...
  if(vrzta == 2) {
    alert(arr[0].komentator_username);
...
function analiza_novi_komentar(response,diid) {
  document.getElementById("komentar_posalji").value = "pošalji";
  var arr = JSON.parse(response);
...
function info_p(rid,sw) {
  var akteri = [];
	...
	if(dgdj_info != 0) {
	   obj_naziv = "t" + dgdj_info;
		//document.getElementById(obj_naziv).innerHTML = " ";
	}
	dgdj_info = rid * 1;
	obj_naziv = "t" + rid;
	if(g_obj_naziv != "xxx") {
  ...
  if(sw==2) {
	...
	if(mi != -1) {
		sadrzaj1 += "<tr><td style='font-style: italic;'>autor:" + "</td><td onclick='prikaz_aktera(" + akteri[0][mi] + ",1)' style='font-weight: bold;' title='klikom dobijete spisak predstava u kojima akter učestvuje'>" +  akteri[1][mi] + " " + akteri[2][mi] + "</td></tr>";
	}
	mi = akteri[3].indexOf(3);
	if(mi != -1) {
		sadrzaj1 += "<tr><td style='font-style: italic;'>reditelj:" + "</td><td onclick='prikaz_aktera(" + akteri[0][mi] + ",1)' style='font-weight: bold;' title='klikom dobijete spisak predstava u kojima akter učestvuje'>" +  akteri[1][mi] + " " + akteri[2][mi] + "</td></tr>";
	}

	for(mi=0;mi<akteri[0].length;mi++) {
		if(akteri[3][mi] == 1) {
			sadrzaj1 += "<tr><td style='font-style: italic;'>igra:" + "</td><td onclick='prikaz_aktera(" + akteri[0][mi] + ",1)' style='font-weight: bold;' title='klikom dobijete spisak predstava u kojima akter učestvuje'>" +  akteri[1][mi] + " " + akteri[2][mi] + "</td></tr>";
		}
	}
	sadrzaj1 += "</table>";
	...
	sadrzaj2 += "<div><input type='button' value='termini igranja, komentari članova Kluba' onclick='prikaz_komentara(" + br_pr +	 ",1);'></td><td class=xbelo></div>"
  for(i=1;i<5;i++) {
      ...
      strsc1 = g_5_skr_kuca[jjj];
        ...
        str_sapopustom = g_3_sapopustom[cc];
  		  ...
  		  dddd + "." + mmmm +  ", " + strsc1 +  " - " + strsc2 +
  			" u " + g_2_hh[j] + ":" + minxxx +
  		  "</td><td title='" + g_2_id[j] + "' onclick='info_p(this.title)'" + boja + "><a href='#' data-rel='popup' class='ui-btn ui-btn-inline ui-corner-all' style='font-size:16px; margin:auto;'>?</a>" +
