...
function brisanje_nenum() {
  ...
  url = g_prefiks + "pozorista.net/np/kartex.php?akcija=brisanjenenum&dogadjaj=" + dgdjidx + "&limit=" + limit + "&ktg=" + kkttgg;
  copyToClipboard(url);
...
function prikazikarte(paket,podig) {
  var redovi = [];
  var brojmesta = paket[0].length;
  var podignuto = podig * 1;
  var ostatak = brojmesta - podignuto;
  var out = "<div>ukupno " + brojmesta + "<br />";
  out += "<div>podignuto " + podignuto + "<br />";
  out += "<div>ostalo " + ostatak + "<br />";
  out += "<div>obri&#353;i <input type='text' size='2' id='zabrisanje' value='0'> nenum karata <input type='button' onclick='brisanje_nenum()'><br />";
...
function prikaz_komentara(arg,vrzta) {
  g_akcija = 99;
  var predstave_k = [];
  var piid = 0;
  var kiid = 0;
  ...
  var redni = -5;
  var podaci = [];
  var tabela_komentara = "";
  var termini_igranja = "";

  if(((vrzta * 1) == 1) || ((vrzta * 1) == 3) || ((vrzta * 1) == 4)) {
    piid = arg;
    if (arg * 1 == 0) {
      piid = xxpredstave[Math.floor(xxpredstave.length * Math.random())];
      if(document.getElementById("hid_pr").value != ".") {
        piid = document.getElementById("hid_pr").value;
      }
    }
    tabela_komentara = koment_var_prikaz(piid,vrzta); // ubacivanje komentara
  }
  if((vrzta * 1) == 2) {
    kiid = arg;
    redni = k_komentator_id.indexOf(kiid + "", 0);
    indik = redni;
    predstave_k.push(k_predstava_id[redni]);
    while(indik != -1) {
      redni = k_komentator_id.indexOf(kiid + "", indik+1);
      indik = redni;
      predstave_k.push(k_predstava_id[redni]);
    }
    piid = predstave_k[Math.floor(predstave_k.length * Math.random())];
    tabela_komentara = koment_var_prikaz(kiid,vrzta); // ubacivanje komentara
  }
  podaci = info_p(piid,2);
  termini_igranja = na_pred(piid);
  var iiid = g_4_id.indexOf(piid * 1);
  document.getElementById("pid01").innerHTML = predstava_obrazac;
  document.getElementById("hid_piid").value = piid;
  document.getElementById("slovna_pretraga").innerHTML = slov_pretr;
  ...
  //copyToClipboard(tabela_komentara);
  if(document.getElementById("pid99") !== null) {
    document.getElementById("pid99").innerHTML = tabela_komentara;
    if((vrzta * 1) == 1) {
      document.getElementById("komentar_posalji").title = iid;
      if(g_korisnik_clbroj != 0) {
        document.getElementById("komentar_clbr").value = g_korisnik_clbroj;
      }
    }
  } else {
	document.getElementById("pid01").innerHTML = tabela_komentara;
    if((vrzta * 1) == 1) {
      document.getElementById("komentar_posalji").title = iid;
      if(g_korisnik_clbroj != 0) {
        document.getElementById("komentar_clbr").value = g_korisnik_clbroj;
...
function prikaz_komentara_old(iid,vrzta) {
  ucitavanje_komentara(iid,vrzta);
  g_akcija = 99;
  var podaci = [];
  var termini_igranja = "";
  diid = iid;
  ...
  if(((vrzta * 1) == 1) || ((vrzta * 1) == 4)) {
    url = g_prefiks + "pozorista.net/np/get_predst_info.php?akcija=komentari_po_predstavi&predstava=" + diid;
  }
  if((vrzta * 1) == 2) {
    url = g_prefiks + "pozorista.net/np/get_predst_info.php?akcija=komentari_po_komentatoru&komentator=" + iid;
  }
  if((vrzta * 1) == 3) {
    url = g_prefiks + "pozorista.net/np/get_predst_info.php?akcija=komentari_najnoviji";
  }
...
function posalji_novi_komentar() {
  ...
  var test2 = 0;
  var test3 = 0;
  document.getElementById("komentar_posalji").value = "sacekajte";
  var pid = document.getElementById("hid_idpr").value;
  var diid = document.getElementById("hid_piid").value;
    ...
    if((test1 == 1) && (test2 == 1) && (test3 == 1)) {
      ...
      xmlhttp_novi_komentar.onreadystatechange=function() {
        if (xmlhttp_novi_komentar.readyState == 4 && xmlhttp_novi_komentar.status == 200) {
          ...
          outp = analiza_svikomentari(aiai,1,diid);
        }
      }
...
function analiza_novi_komentar(response,diid) {
  ...
  if(odgovor == '-1') {
    alert("pogrešan clanski broj!");
  }
  if(odgovor == '-2') {
    alert("inicijali ne odgovaraju članskom broju!");
  }
  if(odgovor * 1 > 0) {
    ucitavanje_komentara(diid,1);
    //koment_var_prikaz(diid,1);
...
var k_komentar_id = [];
var k_tekst_komentara = [];
var k_datum_komentara = [];
var dankk = [];
var meseckk = [];
var godinakk = [];
var minkk = [];
...
var hhkk = [];
var k_predstava_id = [];
var k_predstava_nasl = [];
var k_komentator_id = [];
var k_komentator_username = [];

//dodato

function ucitavanje_komentara(iid,vrzta) {
  var xmlhttp_svikomentari = new XMLHttpRequest();

  url = g_prefiks + "pozorista.net/np/get_predst_info.php?akcija=svi_komentari";
xmlhttp_svikomentari.onreadystatechange=function() {
  if (xmlhttp_svikomentari.readyState == 4 && xmlhttp_svikomentari.status == 200) {
    aiai = xmlhttp_svikomentari.responseText;
    if(aiai == "[]") {
      aiai = '[{"odgovor":"-5"}]';
    }
    outp = analiza_svikomentari(aiai,vrzta,iid);
  }
}
xmlhttp_svikomentari.open("GET", url, true);
xmlhttp_svikomentari.send();

/*
arr[0].predstava_nasl
arr[0].komentator_username
arr[i].predstava_id
arr[i].komentar_id
arr[i].datum_komentara
arr[i].tekst_komentara
*/
}

function analiza_svikomentari(response,vrzta,diid) {
  var arr = JSON.parse(response);
  if(arr.length != 0) {
    for(i=0;i<arr.length;i++) {
      k_komentar_id[i] = arr[i].komentar_id;
      k_predstava_id[i] = arr[i].predstava_id;
      k_predstava_nasl[i] = arr[i].predstava_nasl;
      k_komentator_id[i] = arr[i].komentator_id;
      k_komentator_username[i] = arr[i].komentator_username;
      k_datum_komentara[i] = arr[i].datum_komentara;
      k_tekst_komentara[i] = arr[i].tekst_komentara;
      }
    }
  if(vrzta * 1 != 0) {
    if(document.getElementById("pid99") !== null) {
      document.getElementById("pid99").innerHTML = koment_var_prikaz(diid,vrzta);
      if((vrzta * 1) == 1) {
        document.getElementById("komentar_posalji").title = iid;
        if(g_korisnik_clbroj != 0) {
          document.getElementById("komentar_clbr").value = g_korisnik_clbroj;
...
function koment_prikaz() {
  ...
  var out = unos_komentara_obrazac;
  out += "<input type='hidden' id='hid_idpr' value='" + "0" + "'>";
  out += "<table style='width:100%'><tr><td colspan='" + span4 + "' class=xindigo>komentari &#269;lanova kluba o predstavi " + k_predstava_nasl[0] + "</td></tr><tr><td class=xbelo>";

  for(i=0;i<k_komentar_id.length;i++) {
    xpredst_id = k_predstava_id[i];
    out += "<input type='button' value='" + k_predstava_nasl[i] + "' onclick='prikaz_komentara(" + k_predstava_id[i] +	 ",1);'></td><td class=xbelo>";
    xkomentar_id = k_komentar_id[i];
    xkomentator_id = k_komentator_id[i]
    out += "<input type='button' value='" + k_komentator_username[i] + "' onclick='prikaz_komentara(" + k_komentator_id[i] + ",2);'></td><td class=xbelo>";
    dat_vr_str = k_datum_komentara[i];
    dk = new Date(dat_vr_str);
    godinak = dk.getFullYear();
    meseck = dk.getMonth() + 1;
    dank = dk.getDate();
    ...
    //xrang = k_rang[i];
    //vrxta = k_vrsta[i];
    if(i == k_komentar_id.length-1) {
      out += k_tekst_komentara[i] + "</td></tr></table>";
    } else {
      out += k_tekst_komentara[i] + "</td></tr><tr><td class=xbelo>";
    }
  }
...
function koment_var_prikaz(iid,vrzta) {
  //vrzta = 1 -> po_predstavi -> izostavljen naslov
  //vrzta = 2 -> po_komentatoru -> izostavljen username
  //vrzta = 3 -> komentari_najnoviji -> ima oboje
  //vrzta = 4 -> ?
  if(document.getElementById("pid99") !== null) {document.getElementById("komentar_posalji").value = "posalji";}
  var span3 = "3";
  var span4 = "4";
  var prelom = "";
  var span_plus3 = "";
  ...
  if(((vrzta * 1) == 1) || ((vrzta * 1) == 4)) {
    var out = unos_komentara_obrazac;
    var a = g_4_id.indexOf(iid) * 1;
    out += "<input type='hidden' id='hid_idpr' value='" + iid + "'>";
    out += "<table style='width:100%'><tr><td colspan='" + span3 + "' class=xindigo>komentari &#269;lanova kluba o predstavi " + g_4_nasl_dijakr[a] + "</td></tr><tr><td class=xbelo>";
  }
  if(vrzta == 2) {
    var a = k_komentator_id.indexOf(iid + "");//!!! + ""
	  var out = "<table style='width:100%'><tr><td colspan='" + span3 + "' class=xindigo>komentari komentatora " + '"' + k_komentator_username[a] + '"' + "</td></tr><tr><td class=xbelo>";
  }
  if(vrzta == 3) {
	  var out = "<table style='width:100%'><tr><td colspan='" + span4 + "' class=xindigo>najnoviji komentari</td></tr><tr><td class=xbelo>";
  }
  ...
  for(i=0;i<k_komentar_id.length;i++) {
    switch(vrzta * 1) {
      case 1:
        if(k_predstava_id[i] == iid) {
          //alert(k_predstava_id[i]);
          xpredst_id = k_predstava_id[i];
          if (vrzta!=1) {out += "<input type='button' value='" + k_predstava_nasl[i] + "' onclick='prikaz_komentara(" + k_predstava_id[i] +	 ",1);'></td><td class=xbelo>";}
          xkomentar_id = k_komentar_id[i];
          xkomentator_id = k_komentator_id[i]
          if (vrzta!=2) {out += "<input type='button' value='" + k_komentator_username[i] + "' onclick='prikaz_komentara(" + k_komentator_id[i] + ",2);'></td><td class=xbelo>";}
          dat_vr_str = k_datum_komentara[i];
          dk = new Date(dat_vr_str);
          godinak = dk.getFullYear();
          meseck = dk.getMonth() + 1;
