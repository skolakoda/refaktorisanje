...
function provera_ver() {
  var svakog_sata = setInterval(osvez, 1200000);
}
...
function osvez() {
  ...
  xmlhttp2_ver.send();
  var xmlhttp22_ver = new XMLHttpRequest();
  url2 = "http://www.pozorista.net/np/get_rep.php?akcija=proveri_verziju";
  xmlhttp22_ver.onreadystatechange=function() {
    if (xmlhttp22_ver.readyState == 4 && xmlhttp22_ver.status == 200) {
      aiai = xmlhttp22_ver.responseText;
      outp = analiza_ver(aiai);
      t_verz = outp;
      if(t_verz != g_verz) {
        location.reload(true);
  ...
  xmlhttp22_ver.open("GET", url2, true);
  xmlhttp22_ver.send();
}
