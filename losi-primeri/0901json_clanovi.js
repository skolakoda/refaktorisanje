...
function analiza_cl_rzrv(response) {
  ...
  for(i = 0; i < arr.length; i++) {
    ...
    "</td><td class='belo'><input type='button' value='akcija' onclick='rezerv_akcija(" + arr[i].eid + ")'></td></tr>";
  }
  out += "</table>";
  //copyToClipboard(out);
...
function analiza_za_ucla(response) {
  var arr = JSON.parse(response);
	...
	if(out_clbr>769) {
		alert("čestitamo - učlanili ste se u klub ljubitelja pozorišta. Vaš članski broj je " + out_clbr + "Podatak o članskom broju poslat je na Vašu e-mail adresu.");
		/*
		ima_neko=1;
